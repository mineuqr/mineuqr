...
describe("MIGRATION-GOVERNANCE-RESTORATION-1 regression guards", () => {
  it("journal contains canonical migrations 0000–0085 contiguously", () => {
    ...
    expect(journal.entries[85]?.tag).toBe(CANONICAL_MIGRATION_TAIL_TAG);
  ...
  it("exports certified migration tail constant", () => {
    expect(CANONICAL_MIGRATION_TAIL_TAG).toBe(
      "0085_commercial_catalog_adoption_bindings"
    );
    expect(CANONICAL_JOURNAL_ENTRY_COUNT).toBe(86);
