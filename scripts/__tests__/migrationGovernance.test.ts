...
describe("MIGRATION-GOVERNANCE-RESTORATION-1 regression guards", () => {
  it("journal contains canonical migrations 0000–0084 contiguously", () => {
    ...
    expect(journal.entries[84]?.tag).toBe(CANONICAL_MIGRATION_TAIL_TAG);
    expect(validateJournalOrdering()).toEqual([]);
  });

  it("exports certified migration tail constant", () => {
    expect(CANONICAL_MIGRATION_TAIL_TAG).toBe("0084_commercial_catalog_foundation");
    expect(CANONICAL_JOURNAL_ENTRY_COUNT).toBe(85);
