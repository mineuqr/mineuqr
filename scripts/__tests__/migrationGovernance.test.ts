...
describe("MIGRATION-GOVERNANCE-RESTORATION-1 regression guards", () => {
  it("journal contains canonical migrations 0000–0081 contiguously", () => {
    ...
    expect(journal.entries[81]?.tag).toBe(CANONICAL_MIGRATION_TAIL_TAG);
    expect(validateJournalOrdering()).toEqual([]);
  });

  it("exports certified migration tail constant", () => {
    expect(CANONICAL_MIGRATION_TAIL_TAG).toBe("0081_crmp_financial_shift_number");
    expect(CANONICAL_JOURNAL_ENTRY_COUNT).toBe(82);
