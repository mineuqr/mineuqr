...
function main() {
  ...
  if (journal.entries.length !== CANONICAL_JOURNAL_ENTRY_COUNT) {
    failed = true;
    console.error(
      `\n✗ FAIL — expected ${CANONICAL_JOURNAL_ENTRY_COUNT} journal entries (0000–0098), got ${journal.entries.length}`
    );
  }
  ...
  console.log(
    `\n✓ Journal ↔ SQL lineage consistent (canonical migrations 0000–0098).`
  );
