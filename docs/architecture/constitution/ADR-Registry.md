...
| ADR-ARCH-019 | Order Identity via Service Mode and Fulfilment Anchor | **Accepted** | Architecture Authority | KIOSK-ORDER-IDENTITY-ARCHITECTURE-1 | ORDER-IDENTITY-RUNTIME-1, OPERATIONAL-SESSION-PLATFORM-1, NON-TABLE-PLACE-ORDER-1 | Order identity / PlaceOrder / Session | Partial (runtime + session + non-table PlaceOrder) | Identity PlaceOrder capability active; ops label / channel UI adoption pending |
