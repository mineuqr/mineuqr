...
| ADR-ARCH-019 | Order Identity via Service Mode and Fulfilment Anchor | **Accepted** | Architecture Authority | KIOSK-ORDER-IDENTITY-ARCHITECTURE-1 | ORDER-IDENTITY-RUNTIME-1 | Order identity / PlaceOrder / Session | Partial (runtime foundation) | Generalizes table-only identity; runtime contracts + table dual-write delivered |
