...
| ADR-ARCH-020 | Financial Settlement Platform Architecture | **Accepted** | Architecture Authority | SALES-SETTLEMENT-PLATFORM-ARCHITECTURE-1 · CHECK-GENERALIZATION-ARCHITECTURE-1 | — | Check / Session / Reporting financial SSOT | **Not implemented** | Generalizes Check; membership + optional Session. **Partially superseded by ADR-ARCH-039** on certified adopted channels (Collection Fact = Revenue/PAID; Check remains bill). I-FIN-01/02 remain for all non-adopted collections. No runtime change from 039. |
...
| ADR-ARCH-037 | Payment Process Domain | **Accepted** (governance) | Architecture Authority | PAYMENT-CONSTITUTIONAL-REFINEMENT-1 · PAYMENT-CONFIRM-SERVICE-1 · PAYMENT-CONFIRM-REMAINING-CALLERS-1 · PAYMENT-CONFIRM-COMPATIBILITY-CLEANUP-1 | — | FSP Payment process vs Check aggregate | **Partial (Confirm façade + caller convergence + compat cleanup)** | Refines 020/023/024/026/032. Payment = process owner; Check = sole monetary aggregate **until ADR-ARCH-039 adoption**. Confirm callers enter `confirmPayment`. I-PAY-14 = Check hosts certified execution, not a Confirm bypass. **Refined by ADR-ARCH-038**. **Partially superseded by ADR-ARCH-039** (Collection Fact as monetary AR on certified adopted channels only; runtime unchanged). |
| ADR-ARCH-038 | Cashier Direct Financial Commit Without Pre-Payment Check Readiness | **Accepted** (governance) | Architecture Authority | PAYMENT-CASHIER-DIRECT-FINANCIAL-COMMIT-ARCHITECTURE-1 · PAYMENT-CASHIER-DIRECT-FINANCIAL-COMMIT-IMPLEMENTATION-1 | — | FSP Cashier Confirm sequencing vs OPEN Check readiness | **Partial (runtime; production validation pending)** | Refines 037/020. `cashier_pos` Confirm MUST NOT require a ready/open Check. Check materializes inside synchronous financial commit. I-PAY-19…24. **I-PAY-19 retained by ADR-ARCH-039.** §7 completion set **partially superseded by 039 after certified Cashier adoption**; until then runtime remains §7. |
...
| ADR-ARCH-038 | [ADR-ARCH-038-cashier-direct-financial-commit.md](../adrs/ADR-ARCH-038-cashier-direct-financial-commit.md) |
