...
| ADR-ARCH-038 | Cashier Direct Financial Commit Without Pre-Payment Check Readiness | **Accepted** (governance) | Architecture Authority | PAYMENT-CASHIER-DIRECT-FINANCIAL-COMMIT-ARCHITECTURE-1 | — | FSP Cashier Confirm sequencing vs OPEN Check readiness | **Governance only** | Refines 037/020. `cashier_pos` Confirm MUST NOT require a ready/open Check. Check materializes inside synchronous financial commit. I-PAY-19…24. `ensureCheckForOrder` not globally deprecated. No async commit, PaymentEngine, or payments table. Runtime still gates Confirm on OPEN Check until successor implementation. |
