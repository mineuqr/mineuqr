/**
 * CRMP-PRODUCTION-MIGRATION-0080 — read-only DB smoke (no writes).
 * Validates Catalog columns are selectable (ORM-compatible schema shape).
 */
...
async function main() {
  ...
  try {
    const [registers] = await conn.query(
      `SELECT registerId, status, dutyStatus, code, registerType, archivedAt,
