...
describe("COMPATIBILITY-CLEANUP-1 ops settlement APIs removed", () => {
  it("does not expose getSettlement* procedures", () => {
    const caller = createVerifiedCaller();
    expect(caller.ops).not.toHaveProperty("getSettlementSummary");
    expect(caller.ops).not.toHaveProperty("getSettlementBreakdown");
    expect(caller.ops).not.toHaveProperty("getSettlementTrend");
  });
});
